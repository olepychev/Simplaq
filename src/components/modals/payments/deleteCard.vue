    ...
    <div class="w-full h-full flex items-center justify-center">
      <div class="w-full max-w-[664px] px-[40px] pt-[56px] pb-[40px] bg-white rounded-[24px]">
        <div class="w-full h-full">
          <div class="flex relative mx-auto mb-[24px] bg-gray rounded-full w-[72px] h-[72px] flex items-center justify-center gap-[10px]">
            <Icon icon="fluent:delete-28-regular" class="text-lg text-black" />
          </div>

          <h6 class="font-bold text-black text-xl mb-[8px] text-center">{{ $t('delete_payment_method') }}</h6>

          <p class="max-w-[342px] w-full mx-auto font-medium text-sm text-grayDark text-center leading-[24px]">{{ $t('are_you_sure_you_want_to_remove_your_card') }}</p>
...
<script lang="ts">
    export default {
        name: "DeleteCard",
        emits: ['deleteCard', 'cancelDeleteCard'],
        methods: {
            deleteCard() {
                this.$emit('deleteCard')
            },
            cancelDeleteCard() {
                this.$emit('cancelDeleteCard')
            }
        }
    }
</script>