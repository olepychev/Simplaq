        ...
        <div v-if="!formSended" class="w-full h-full">
          <div class="flex items-center justify-between pb-[16px]">
            <div class="flex flex-col gap-[4px]">
              ...
              <p class="text-grayDark text-sm leading-[24px] tracking-[0.1px] font-normal">{{
                $t('please_describe_the_issue_below') }}</p>
            </div>
          ...
          <form class="flex flex-col gap-[8px] w-full" id="my-dropzone">
            <div v-if="!userStore.token" class="grid sm:grid-cols-2 gap-[8px]">
              ...
              <div
                ...
                <div class="flex flex-col gap-[2px]">
                  <input @input="nameTouched" type="text" id="full_name" :placeholder="$t('full_name')"
                    @keydown.enter.prevent="handleSubmit" v-model="userData.name"
                    class="text-black py-[22px] font-medium bg-transparent outline-none text-sm leading-[20px]" />
                </div>
                <div>
            ...
            <div
              ...
              <div class="flex flex-col gap-[2px]">
                <textarea maxlength="150" @input="messageTouched" type="text" @keydown.enter.prevent="handleSubmit"
                  id="email" :placeholder="$t('message')" v-model="userData.message"
                  class="h-[100px] text-black font-medium resize-none bg-transparent outline-none text-sm leading-[20px]"></textarea>
              </div>
              <div>
            ...
            <div class="flex gap-[8px] mt-[24px] w-full items-center justify-center">
              <button type="button" @click="this.$emit('closeModal')"
  ...
  computed: {
    readyForSubmit(): boolean {
      if (this.userStore.token) {
        ...
        if (this.userData.name.length === 0 || this.userData.email.length === 0 || this.userData.message.length === 0 && this.userData.message.length <= 150) {
          return false
        } else {
          return true
        }
