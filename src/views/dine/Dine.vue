<template>
    ...
    <div class="w-full relative flex py-[16px] max-w-[1600px] mx-auto px-[25px] gap-[40px]">
      <div class="w-full relative mt-[16px]">
        <h6 class="text-black font-bold text-2xl leading-[38px] mb-[20px] tracking-[-0.4px]">{{ $t('dine') }}</h6>
        <div class="w-full flex items-center justify-between gap-[50px] mb-[20px]">
          <ul class="flex items-center gap-[8px]">
            <li
              @click="filterDine"
              dine-filter="all"
              :class="dineFilter === 'all' ? 'bg-orange text-white' : 'bg-gray text-black'"
            ...
            <li
              @click="filterDine"
              :dine-filter="DineFilter.CAFE"
              :class="dineFilter === DineFilter.CAFE ? 'bg-orange text-white' : 'bg-gray text-black'"
              class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
            >
              {{ $t('cafe') }}
            </li>
            <li
              @click="filterDine"
              :dine-filter="DineFilter.RESTAURANT"
              :class="dineFilter === DineFilter.RESTAURANT ? 'bg-orange text-white' : 'bg-gray text-black'"
              class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
            >
              {{ $t('restaurant') }}
            </li>
            <li
              @click="filterDine"
              :dine-filter="DineFilter.SUSHI_BAR"
              :class="dineFilter === DineFilter.SUSHI_BAR ? 'bg-orange text-white' : 'bg-gray text-black'"
              class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
            >
              {{ $t('sushi_bar') }}
            </li>
            <li
              @click="filterDine"
              :dine-filter="DineFilter.COFFEE"
              :class="dineFilter === DineFilter.COFFEE ? 'bg-orange text-white' : 'bg-gray text-black'"
              class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
            >
              {{ $t('coffee') }}
            </li>
            <li
              @click="filterDine"
              :dine-filter="DineFilter.SEA_FOOD"
              :class="dineFilter === DineFilter.SEA_FOOD ? 'bg-orange text-white' : 'bg-gray text-black'"
              class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
            >
              {{ $t('sea_food') }}
            </li>
            <li
              @click="filterDine"
              :dine-filter="DineFilter.HEALTHY_FOOD"
              :class="dineFilter === DineFilter.HEALTHY_FOOD ? 'bg-orange text-white' : 'bg-gray text-black'"
              class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
            >
              {{ $t('healthy_food') }}
            </li>
        ...
        <div v-if="dynamicDineArr.length > 0">
          <div class="w-full relative mt-[16px]">
            <div class="w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-[16px] gap-y-[24px]">
              <RouterLink :to="{ name: $Routes.DINE_SINGLE, params: { id: 1 } }" class="relative w-full flex flex-col shadow-sm">
    ...
    <FilterComponent v-if="activefilter" @closeFilterComponent="closeFilterComponent">
      ...
      <div class="w-full flex flex-col gap-[12px] mt-[16px]">
        <p class="text-black font-bold text-base leading-[24px] capitalize">{{ $t('food') }}</p>
        <div class="flex w-full flex-wrap gap-[8px]">
          <label class="cursor-pointer">
            <input type="checkbox" name="" id="" class="peer sr-only custom-peer" />
            <div
              class="flex items-center justify-center custom-peer-checked peer-checked:bg-orange gap-[2px] bg-gray px-[24px] py-[12px] rounded-[12px]"
            >
              <p class="font-semibold text-xs leading-[16px] text tracking-[-0.1px] text-black peer-checked:text-white">{{ $t('desert') }}</p>
            </div>
          </label>
          <label class="cursor-pointer">
            <input type="checkbox" name="" id="" class="peer sr-only custom-peer" />
            <div
              class="flex items-center justify-center custom-peer-checked peer-checked:bg-orange gap-[2px] bg-gray px-[24px] py-[12px] rounded-[12px]"
            >
              <p class="font-semibold text-xs leading-[16px] text tracking-[-0.1px] text-black peer-checked:text-white">{{ $t('street_food') }}</p>
            </div>
          </label>
          <label class="cursor-pointer">
            <input type="checkbox" name="" id="" class="peer sr-only custom-peer" />
            <div
              class="flex items-center justify-center custom-peer-checked peer-checked:bg-orange gap-[2px] bg-gray px-[24px] py-[12px] rounded-[12px]"
            >
              <p class="font-semibold text-xs leading-[16px] text tracking-[-0.1px] text-black peer-checked:text-white">{{ $t('sea_food') }}</p>
            </div>
          </label>
          <label class="cursor-pointer">
            <input type="checkbox" name="" id="" class="peer sr-only custom-peer" />
            <div
              class="flex items-center justify-center custom-peer-checked peer-checked:bg-orange gap-[2px] bg-gray px-[24px] py-[12px] rounded-[12px]"
            >
              <p class="font-semibold text-xs leading-[16px] text tracking-[-0.1px] text-black peer-checked:text-white">{{ $t('healthy_food') }}</p>
            </div>
          </label>
          <label class="cursor-pointer">
            <input type="checkbox" name="" id="" class="peer sr-only custom-peer" />
            <div
              class="flex items-center justify-center custom-peer-checked peer-checked:bg-orange gap-[2px] bg-gray px-[24px] py-[12px] rounded-[12px]"
            >
              <p class="font-semibold text-xs leading-[16px] text tracking-[-0.1px] text-black peer-checked:text-white">{{ $t('pizza') }}</p>
            </div>
          </label>
          <label class="cursor-pointer">
            <input type="checkbox" name="" id="" class="peer sr-only custom-peer" />
            <div
              class="flex items-center justify-center custom-peer-checked peer-checked:bg-orange gap-[2px] bg-gray px-[24px] py-[12px] rounded-[12px]"
            >
              <p class="font-semibold text-xs leading-[16px] text tracking-[-0.1px] text-black peer-checked:text-white">{{ $t('burger') }}</p>
            </div>
          </label>
        </div>
      </div>
  ...
  import { DineFilter } from '@/enums/dine/dineFilter.ts'
  ...
  export default {
    ...
    data() {
      return {
        ...
        DineFilter: DineFilter,
        dineFilter: 'all' as string,
