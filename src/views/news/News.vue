<template>
  ...
  <div class="w-full relative flex py-[16px] max-w-[1600px] mx-auto px-[25px] gap-[40px]">
    <div class="w-full relative mt-[16px]">
      <h6 class="text-black font-bold text-2xl leading-[38px] mb-[20px] tracking-[-0.4px]">{{ $t('news') }}</h6>
      <div class="w-full flex items-center justify-between gap-[50px] mb-[20px]">
        <ul class="flex items-center gap-[8px]">
          <li
            @click="filterNews"
            news-filter="all"
            :class="newsFilter === 'all' ? 'bg-orange text-white' : 'bg-gray text-black'"
          ...
          <li
            @click="filterNews"
            :news-filter="NewsFilter.MALL_NEWS"
            :class="newsFilter === NewsFilter.MALL_NEWS ? 'bg-orange text-white' : 'bg-gray text-black'"
            class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
          >
            {{ $t('mall_news') }}
          </li>
          <li
            @click="filterNews"
            :news-filter="NewsFilter.STORE_NEWS"
            :class="newsFilter === NewsFilter.STORE_NEWS ? 'bg-orange text-white' : 'bg-gray text-black'"
            class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
          >
            {{ $t('store_news') }}
          </li>
          <li
            @click="filterNews"
            :news-filter="NewsFilter.ARTICLE"
            :class="newsFilter === NewsFilter.ARTICLE ? 'bg-orange text-white' : 'bg-gray text-black'"
            class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
          >
            {{ $t('articles') }}
          </li>
          <li
            @click="filterNews"
            :news-filter="NewsFilter.PARTY"
            :class="newsFilter === NewsFilter.PARTY ? 'bg-orange text-white' : 'bg-gray text-black'"
            class="px-[16px] py-[10px] rounded-[12px] text-sm font-semibold leading-[20px] tracking-[-0.2px] cursor-pointer"
          >
            {{ $t('party') }}
          </li>
      ...
      <div v-if="dynamicNewsArr.length > 0">
        <GridArticles>
          <template #main>
          ...
          <template #articles>
            <div class="w-full flex flex-col gap-[20px]">
              <p class="text-orange text-sm font-bold text-orange">{{ $t('popular_articles') }}</p>
...
import { NewsFilter } from '@/enums/news/newsFilter.ts'
...
export default {
  ...
  methods: {
    filterNews(e: Event) {
      const target = e.target as Element
      const targetFilter = target.closest('li').getAttribute('news-filter')
      this.newsFilter = targetFilter
    },
    clearSearchInput() {
      this.searchVal = ''
